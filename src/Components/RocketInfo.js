...
const RocketInfo = () => {
    ...
    return ReactDOM.createPortal(
        ...
        <div className='flex fixed top-[50%] left-[50%] transform -translate-x-1/2 -translate-y-1/2 z-50 justify-center xl:w-[71.5rem] lg:w-[60rem] md:w-[46rem] xl:h-[35rem] h-[30rem] drop-shadow-2xl bg-white xl:p-4 rounded-2xl overflow-y-scroll'>
            <div className='flex flex-col xl:h-[35.5rem] h-[32rem] xl:mt-10 mt-8'>
                {photoCheck ? <div className='flex justify-between'><div className='flex gap-6 xl:-ml-[7.5rem] -ml-[5rem] xl:-mt-7'>
                    ...
                    <p className='text-[#656565] text-2xl hover:bg-[#f5f5f5] hover:rounded-lg hover:cursor-pointer xl:block lg:block md:hidden' onClick={() => {
                        setPhotoCheck(true);
                        setOverviewCheck(false);
                    }}>Photos</p>

                </div>
                    <img src={cross} alt="cross" className='-mt-3 hover:cursor-pointer hover:scale-y-105 hover:transform hover:transition-transform hover:duration-200 xl:-mr-0 lg:-mr-0 md:-mr-20' onClick={() => dispatch(checkPopup(false))} />
                </div>}
